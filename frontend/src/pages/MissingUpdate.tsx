...
import { GetMissingUpdates } from "@/wailsjs/go/main/App";
...
export default function MissingUpdate() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await GetMissingUpdates();
            setItems(data || []);
        } catch (err) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold tracking-tight">Missing Updates</h2>
                                ...
                                {filteredItems.length === 0 ? (
                                    ...
                                    filteredItems.map((item: process.IncompleteTitle, index: number) => (
                                        <TableRow key={item.Attributes.id}>
                                            ...
                                            <TableCell>{item.local_update}</TableCell>
                                            <TableCell>{item.latest_update}</TableCell>
                                            <TableCell>{item.latest_update_date}</TableCell>
                                        </TableRow>
                                    ))
                                )}
                            </TableBody>
